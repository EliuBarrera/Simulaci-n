...
public class LeyGaussController {
    ...
    @FXML private TextField   figCxField, figCyField, figCzField;
    @FXML private TextField   figP1Field, figP2Field, figP3Field;
    @FXML private VBox        panelDimensiones3DSuperficie;
    @FXML private TextField   supCxField, supCyField, supCzField;
    @FXML private TextField   supP1Field, supP2Field, supP3Field;
    ...
    private void configurarToggleCarga() {
        ...
        if (optCargaExacta == null) return;
        optCargaExacta.setToggleGroup(toggleCargaGroup);
        optDensidad.setToggleGroup(toggleCargaGroup);
        toggleCargaGroup.selectedToggleProperty().addListener((obs, o, n) ->
            lblCargaInfo.setText(optCargaExacta.isSelected()
                ? "Carga Q (µC):"
                : "Densidad (µC/m, m², m³):")
        );
    }
    ...
    private void configurarEscena3D() {
        if (gaussPane == null || gaussScrollPane == null) return;
    ...
    private void alternarVistaPrincipal() {
        ...
        if (sub) {
            ...
            if (modo3D) manager3D.getGenerador().resetCamera();
        }
        if (gaussScrollPane != null) gaussScrollPane.setPannable(!modo3D);
    }
    ...
    private void onIniciarDibujoFigura() {
        if (modo3D) { crearFigura3DDirecto(); return; }

        if (!validarCampoNumerico(cargaField, "Ingrese el valor antes de dibujar.")) return;
    ...
    private void onIniciarDibujoSuperficie() {
        if (modo3D) { crearSuperficie3DDirecto(); return; }
    ...
    private void onLimpiar() {
        ...
        if (manager3D != null) manager3D.getGenerador().limpiarElementos();
    ...
    private void onMouseDragged(MouseEvent e) {
        double px = e.getX(), py = e.getY();
        if (esperandoDibujo) {
            dragCurrentX = px; dragCurrentY = py;
            renderConPreview();
            return;
        }
    ...
    private void onMouseReleased(MouseEvent e) {
        if (modoActual == Modo.MOVIENDO) {
            figuraMoviendo = null; modoActual = Modo.NADA;
            canvasGauss.setCursor(Cursor.DEFAULT);
            return;
        }
    ...
    private void redibujar() {
        if (modo3D) {
            if (manager3D != null) manager3D.actualizar(figuraCargada, superficieGauss);
        } else {
    ...
    private double calcularCargaTotal(double valor, TipoFigura tipo,
        ...
        if (optCargaExacta != null && optCargaExacta.isSelected()) return valor;
    ...
    private boolean validarCampoNumerico(TextField field, String mensaje) {
        try {
            ...
            if (v <= 0) { mostrarAlerta("Error", "El valor debe ser positivo."); return false; }
            return true;
        } catch (NumberFormatException e) {
